  ...
  return (
    <Form>
      {items.length > 0 ? (
        ...
        <EmptyListMessage id="no_destinations_added" />
      )}
      {percentageValid && (
        <Info
          mt={2}
          noMargin
          variant="warning"
          id="sum_off_percentages_current_total_title"
          langContext={{ percentageTotal }}
        />
      )}

      <Box
        pt={percentageValid ? 2 : 1}
...
function PayoutDestination(props) {
